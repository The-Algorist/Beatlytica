...
BIGQUERY_DATASET = os.environ.get('BIGQUERY_DATASET', 'beatlytica_stg')
...
with DAG(
    dag_id = f'beatlytica_stream_dag',
    default_args = default_args,
    description = f'Hourly data pipeline to generate dims and facts for beatlytica',
    schedule_interval="0 */10 * * * *", #Runs every 10 minutes
    start_date=datetime(2023,4,20,12),
    catchup=False,
    max_active_runs=1,
    ...
    execute_dbt_task = BashOperator(
        task_id = 'dbt_beatlytica_stream_run',
